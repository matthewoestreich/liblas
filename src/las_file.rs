...
impl LasFile {
  pub fn parse(file_path: PathBuf) -> Result<Self, LibLasError> {
    ...
    while let Some(read_line) = line_reader.next() {
      ...
      if current_line.starts_with(&Token::VersionInformationSection()) {
      ...
      if current_line.starts_with(&Token::WellInformationSection()) {
      ...
      if current_line.starts_with(&Token::OtherSection()) {
      ...
      if current_line.starts_with(&Token::ParameterInformationSection()) {
      ...
      if current_line.starts_with(&Token::CurveInformationSection()) {
      ...
      if current_line.starts_with(&Token::AsciiSection()) {
